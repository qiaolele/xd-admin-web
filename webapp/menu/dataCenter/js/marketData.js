...
OperatingAndApi.regEvent = function () {
  $("body").on("click", ".startTime", function () {
    ...
    $.get("/app/admin/v1/record/center/agency/api/update", params, function (
      data
    ) {
      if (data.code === 200) {
        parent.modal.operModal({
...
OperatingAndApi.getDataList = function (param) {
  ...
  $.get("/app/admin/v1/record/center/appmarket/query", param, function (data) {
      ...
      if (d.length > 0) {
        d.map(function (item, index) {
          html += `
                  <tr>
                    <td>${item.channel}</td>
                    <td>${item.appMarketLoginCount}</td>
                    <td>${item.appMarketNewRegistrationCount}</td>
