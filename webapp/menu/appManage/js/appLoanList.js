const AppLoanList = new Object(),
  userId = Cookie.get('userId')

AppLoanList.regEvent = function() {
  $('body').on('click', '.check', function() {
    AppLoanList.getParams()
  })
  //选择产品
  $('body').on('click', '.transfer-left .transfer-item', function(e) {
    ...
    if ($checbox.is(':checked')) {
      ...
      const $item = $hideItem.clone().removeClass('hide'),
        data = AppLoanList.productData.filter(
  ...
  $('body').on(
    ...
    function() {
      ...
      AppLoanList.getAddOrEditParams()
    }
  )
  //删除
  ...
  $(parent.window.document).on(
    'click',
    '#operModal.delete .modal-footer .btn:eq(1)',
    function() {
      AppLoanList.deletepage({
        id: $(this)
          .parents('#operModal')
          .attr('data-id'),
        userId
...
//计算位置
AppLoanList.calculation = function() {
...
AppLoanList.getParams = function(page) {
  parent.modal.loaders('block')
  ...
  params.page = page || 1
  params.rows = 25
  AppLoanList.query(params)
}
AppLoanList.query = function(param) {
  $.get('/admin/v1/apploanlist/query', param, info => {
    AppLoanList.createTable(info.data)
    initPage(info.data.total, param, AppLoanList)
  })
}
AppLoanList.createTable = function(info) {
...
//新增、修改
AppLoanList.addandupdate = function(param) {
  $.post('/admin/v1/apploanlist/addandupdate', param, function(info) {
...
//获取新增、修改参数
AppLoanList.getAddOrEditParams = function() {
  const json = {}
  const list = new Array(),
    param = {
      userId,
  ...
  if (id) param.id = id
  AppLoanList.addandupdate(param)
}
//获取产品列表
AppLoanList.getproductlist = function() {
  let param = {
    type: $('.transfer-check')
...
//删除产品
AppLoanList.deletepage = function(param) {
